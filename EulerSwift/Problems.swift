...
struct Problems {
    ...
    public static var problems:[([String], () -> String)] = [
        ...
        (["Problem 14", "Which starting number, under one million, produces the longest collatz chain?"], problem014),
        //(["Problem 15", "How many such routes are there through a 20×20 grid?"], problem015),
        (["Problem 16", "What is the sum of the digits of the number 21000?"], problem016),
        (["Problem 17", "If all the numbers from 1 to 1000 (one thousand) inclusive were written out in words, how many letters would be used?"], problem017)
    ]
    ...
    static func problem011_getProduct(grid:[[Int]], coords:(Int, Int), mode:problem011_AdjacentType) -> Int {
        var product = 1
        if mode == .Diagonal {
            for i in 0 ... 3 {
                product *= grid[coords.1 + i][coords.0 + i]
            }
        }
        if mode == .DiagonalUp {
            for i in 0 ... 3 {
            ...
            for i in 0 ... 3 {
                product *= grid[coords.1][coords.0 + i]
            }
        }
        else if mode == .Vertical {
            for i in 0 ... 3 {
                product *= grid[coords.1 + i][coords.0]
            }
        }
        return product
    }
    static func problem011() -> String {
        ...
        for y in 0 ... 19 {
            for x in 0 ... 19 {
                if y <= 16 && x <= 16 {
                    let p = problem011_getProduct(grid: grid, coords: (x, y), mode: .Diagonal)
                    if p > largestProduct {
                        largestProduct = p
                    }
                }
                if y >= 3 && x <= 16 {
                    let p = problem011_getProduct(grid: grid, coords: (x, y), mode: .DiagonalUp)
                    if p > largestProduct {
                        largestProduct = p
                    }
                }
                if y <= 16 {
                    let p = problem011_getProduct(grid: grid, coords: (x, y), mode: .Vertical)
                    if p > largestProduct {
                        largestProduct = p
                    }
                }
                if x <= 16 {
                    let p = problem011_getProduct(grid: grid, coords: (x, y), mode: .Horizontal)
                    if p > largestProduct {
                        largestProduct = p
                    }
                }
    ...
    static func problem013() -> String {
        ...
        do {
            ...
            return String(str[str.startIndex ... str.index(str.startIndex, offsetBy: 10)])
        }
        catch {
            return "Something went wrong"
        }
    }
    static func problem014() -> String {
        var sequences:Dictionary<Int, Int> = [:] // length, number
        for i in 1 ... 1000000 {
            let s = EulerMaths.collatzLength(start: i)
            sequences.updateValue(i, forKey: s)
        }
        return String(sequences[sequences.keys.max()!]!)
    }
//    static func problem015() -> String {
//        return ""
//    }
    static func problem016() -> String {
        if !BigNumber.test() {
            return "Test failed"
        }
        var n = try! BigNumber(number: "2")
        for _ in 1 ... 999 {
            n = n * 2
        }
        var sum = 0
